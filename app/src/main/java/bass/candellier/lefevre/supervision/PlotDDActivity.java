...
public class PlotDDActivity extends ActionBarActivity {
	...
	public static final String LISTE_DD_KEY = "dd";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for(Number anUsageDDY : usageDDY) {
			if(anUsageDDY.floatValue() < min) {
				min = anUsageDDY.floatValue();
			}

			if(anUsageDDY.floatValue() > max) {
				max = anUsageDDY.floatValue();
			}
		}
		...
		for(int i = 0; i < usageDDY.length; i++) {
			if(i > 0) {
		...
		plot.setTitle("Usages du disque du " + resRequete.get(0).getSdate() + " au " + resRequete.get(resRequete.size() - 1)
				.getSdate());
		plot.setRangeLabel("%");
		...
		plot.addSeries(courbe_dd, new LineAndPointFormatter(Color.rgb(255, 0, 0), Color.rgb(0, 0, 0), Color.argb(20, 255, 0, 0),
