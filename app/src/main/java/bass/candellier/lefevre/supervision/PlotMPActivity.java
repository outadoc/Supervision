...
public class PlotMPActivity extends ActionBarActivity {
	...
	public static final String LISTE_MP_KEY = "mp";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for(Number anUsageMPY : usageMPY) {
			if(anUsageMPY.floatValue() < min) {
				min = anUsageMPY.floatValue();
			}

			if(anUsageMPY.floatValue() > max) {
				max = anUsageMPY.floatValue();
			}
		}
		...
		for(int i = 0; i < usageMPY.length; i++) {
			if(i > 0) {
		...
		plot.setTitle("Usages des processeurs du " + resRequete.get(0).getSdate() + " au " + resRequete.get(resRequete.size() -
				1).getSdate());
         ...
         * sauf si le min est inférieur à 5 ou si le max est supérieur à 95, afin de je ne pas avoir des valeurs incohérentes
         * sur l'axe Y
		...
		plot.addSeries(courbe_mp, new LineAndPointFormatter(Color.rgb(255, 0, 0), Color.rgb(0, 0, 0), Color.argb(20, 255, 0, 0),
				new PointLabelFormatter(Color.BLACK)));
	}
